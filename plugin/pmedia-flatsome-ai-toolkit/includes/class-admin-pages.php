...
final class PMFAI_Admin_Pages
{
    ...
    public static function design_system(): void
    {
        ...
        self::beauty_preset_select($options, true);
    ...
    public static function generate_block(): void
    {
        ...
        echo '</select></label><label class="pmfai-field"><span>Cost / Quality Mode</span><select id="pmfai-generate-cost-mode"><option value="fast">Fast / Cheap</option><option value="balanced" selected>Balanced</option><option value="high">High Quality</option></select></label>';
        self::beauty_preset_select($options, false, 'pmfai-generate-beauty-preset');
        echo '<label class="pmfai-field"><span>Ngành nghề</span><input id="pmfai-generate-industry" value="doanh nghiệp dịch vụ"></label><label class="pmfai-field"><span>Phong cách</span><input id="pmfai-generate-style" value="hiện đại, chuyên nghiệp"></label><label class="pmfai-field"><span>Mục tiêu</span><input id="pmfai-generate-goal" value="Tạo HTML Block copy vào Flatsome"></label></div><div class="pmfai-mode-help"><strong>Gợi ý:</strong> Fast dùng cho section đơn giản; Balanced dùng hằng ngày; High Quality dùng cho hero/landing page quan trọng.</div><label class="pmfai-field"><span>Nội dung / mô tả block</span><textarea id="pmfai-generate-content" rows="7"></textarea></label><p><button class="button button-primary" id="pmfai-generate-block">Generate Block</button></p><div id="pmfai-generate-result"></div></div></div>';
    }
    ...
    public static function page_builder_ai(): void
    {
        ...
        echo '<div class="pmfai-panel"><div class="pmfai-grid-2"><label class="pmfai-field"><span>Build mode</span><select id="pmfai-page-build-mode"><option value="full-page">Sinh cả trang</option><option value="single-section">Sinh từng section</option></select></label><label class="pmfai-field"><span>Output mode</span><select id="pmfai-page-output-mode"><option value="html-block" selected>Flatsome Section + HTML Block</option><option value="flatsome-native">Flatsome Native Shortcode nhiều hơn</option></select></label><label class="pmfai-field"><span>Cost / Quality Mode</span><select id="pmfai-page-cost-mode"><option value="fast">Fast / Cheap</option><option value="balanced" selected>Balanced</option><option value="high">High Quality</option></select></label>';
        self::beauty_preset_select($options, false, 'pmfai-page-beauty-preset');
        echo '</div><div class="pmfai-mode-help"><strong>Beauty Preset:</strong> chọn gu giao diện trước khi sinh. Ví dụ thi trắc nghiệm nên dùng Education Friendly; SaaS dùng Soft SaaS; spa dùng Luxury Gold.</div><div class="pmfai-mode-help"><strong>Output mode:</strong> HTML Block an toàn hơn; Native Shortcode dễ chỉnh trong UX Builder hơn nhưng cần kiểm tra kỹ shortcode Flatsome.</div><label class="pmfai-field"><span>Brief trang</span><textarea id="pmfai-page-brief" rows="8" placeholder="Ví dụ: Tạo trang chủ cho công ty thiết kế website, gồm hero, dịch vụ, quy trình, dự án, FAQ, CTA..."></textarea></label><p><button class="button button-primary" id="pmfai-page-generate">Generate Page</button> <button class="button" id="pmfai-page-bridge">Tạo prompt ChatGPT</button></p><textarea id="pmfai-page-prompt" rows="10" readonly></textarea></div>';
    ...
    public static function settings(): void
    {
        $options = PMFAI_Settings::get_options();
        echo '<div class="wrap pmfai-wrap">';
        self::header('Settings', 'Chọn AI provider và cấu hình API cho Auto Mode. Bridge/Manual Mode không gọi API plugin.');
        echo '<form method="post" action="options.php"><div class="pmfai-panel"><h2>AI Provider</h2>'; settings_fields('pmfai_settings_group');
        self::provider_select($options);
        echo '<div class="pmfai-grid-2">';
        foreach (['api_endpoint'=>'OpenAI endpoint','api_key'=>'OpenAI API key','api_model'=>'OpenAI model'] as $key => $label) { self::field($key, $label, $key === 'api_key' ? 'password' : 'text'); }
        foreach (['compatible_endpoint'=>'OpenAI-compatible endpoint','compatible_api_key'=>'OpenAI-compatible API key','compatible_model'=>'OpenAI-compatible model'] as $key => $label) { self::field($key, $label, strpos($key, 'api_key') !== false ? 'password' : 'text'); }
        foreach (['anthropic_endpoint'=>'Anthropic endpoint','anthropic_api_key'=>'Anthropic API key','anthropic_model'=>'Anthropic model'] as $key => $label) { self::field($key, $label, strpos($key, 'api_key') !== false ? 'password' : 'text'); }
        self::field('temperature', 'Temperature mặc định');
        echo '</div><h2>Beauty Preset mặc định</h2><p class="description">Preset này được dùng cho Page Builder AI, Generate Block và ChatGPT Bridge nếu không chọn riêng từng lần.</p><div class="pmfai-grid-2">';
        self::beauty_preset_select($options, true);
        echo '</div><h2>Model theo Cost / Quality Mode</h2><p class="description">Nếu để trống model của từng mode, plugin sẽ dùng model mặc định của provider đang chọn.</p><div class="pmfai-grid-2">';
        foreach (['fast_model'=>'Fast / Cheap model','fast_temperature'=>'Fast / Cheap temperature','balanced_model'=>'Balanced model','balanced_temperature'=>'Balanced temperature','high_model'=>'High Quality model','high_temperature'=>'High Quality temperature'] as $key => $label) { self::field($key, $label); }
        echo '</div><label class="pmfai-field"><span>Rule bổ sung</span><textarea name="' . esc_attr(PMFAI_OPTION_KEY . '[extra_rules]') . '" rows="5">' . esc_textarea($options['extra_rules']) . '</textarea></label><p>'; submit_button('Lưu Settings', 'primary', 'submit', false); echo '</p></div></form></div>';
    }

    public static function chatgpt_bridge(): void
    {
        $options = PMFAI_Settings::get_options();
        echo '<div class="wrap pmfai-wrap">'; self::header('ChatGPT Bridge', 'Tạo prompt chuẩn để dùng ChatGPT web, giảm chi phí API plugin.'); echo '<div class="pmfai-panel"><div class="pmfai-grid-2"><label class="pmfai-field"><span>Loại yêu cầu</span><select id="pmfai-bridge-type">'; foreach (PMFAI_Prompt_Builder::types() as $key => $label) { echo '<option value="' . esc_attr($key) . '">' . esc_html($label) . '</option>'; } echo '</select></label>';
        self::beauty_preset_select($options, false, 'pmfai-bridge-beauty-preset');
        echo '<label class="pmfai-field"><span>Ngành nghề</span><input id="pmfai-bridge-industry" value="doanh nghiệp dịch vụ"></label><label class="pmfai-field"><span>Phong cách</span><input id="pmfai-bridge-style" value="hiện đại, chuyên nghiệp"></label><label class="pmfai-field"><span>Mục tiêu</span><input id="pmfai-bridge-goal" value="Tạo HTML Block copy vào Flatsome"></label></div><div class="pmfai-mode-help"><strong>Beauty Preset:</strong> prompt gửi sang ChatGPT/Claude sẽ yêu cầu AI sinh đúng gu này.</div><label class="pmfai-field"><span>Nội dung / mô tả</span><textarea id="pmfai-bridge-content" rows="5"></textarea></label><p><button class="button button-primary" id="pmfai-build-prompt">Tạo prompt</button> <button class="button pmfai-copy" data-target="pmfai-bridge-output">Copy prompt</button></p><textarea id="pmfai-bridge-output" rows="18" readonly></textarea></div></div>';
    }
    ...
    private static function beauty_preset_select(array $options, bool $for_settings = false, string $id = ''): void
    {
        $current = $options['beauty_preset'] ?? 'corporate_blue';
        $name = $for_settings ? PMFAI_OPTION_KEY . '[beauty_preset]' : '';
        $attr_name = $name ? ' name="' . esc_attr($name) . '"' : '';
        $attr_id = $id ? ' id="' . esc_attr($id) . '"' : '';
        echo '<label class="pmfai-field"><span>Beauty Preset</span><select' . $attr_id . $attr_name . '>';
        foreach (PMFAI_Beauty_Presets::options_for_select() as $value => $label) {
            echo '<option value="' . esc_attr($value) . '" ' . selected($current, $value, false) . '>' . esc_html($label) . '</option>';
        }
        echo '</select><small class="description">Chọn gu thiết kế để AI sinh giao diện đẹp và nhất quán hơn.</small></label>';
    }

    private static function provider_select(array $options): void
    {
        $name = PMFAI_OPTION_KEY . '[ai_provider]';
        echo '<label class="pmfai-field"><span>AI Provider</span><select name="' . esc_attr($name) . '">';
        foreach (['openai' => 'OpenAI / ChatGPT', 'openai_compatible' => 'OpenAI-compatible', 'anthropic' => 'Anthropic Claude'] as $value => $label) {
            echo '<option value="' . esc_attr($value) . '" ' . selected($options['ai_provider'] ?? 'openai', $value, false) . '>' . esc_html($label) . '</option>';
        }
        echo '</select></label>';
    }
