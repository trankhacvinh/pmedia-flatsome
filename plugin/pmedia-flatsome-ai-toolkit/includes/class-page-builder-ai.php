...
final class PMFAI_Page_Builder_AI
{
    ...
    public static function bridge_prompt(array $params = []): string
    {
        ...
        return "Bạn là senior UI engineer cho WordPress Flatsome.\n\n"
            . "Hãy tạo page plan và các section cho website theo brief.\n"
            . "{$output_rule}\n\n"
            . "Yêu cầu bắt buộc:\n"
    ...
    public static function generate(array $params)
    {
        ...
        $payload = [
            ...
            'messages' => [
                ['role' => 'system', 'content' => 'You generate strict valid JSON page structures for WordPress Flatsome. Return JSON only. Escape quotes exactly once. Reuse design tokens and avoid unnecessary CSS.'],
    ...
    public static function parse_json(string $raw, array $extra = [])
    {
        ...
        if (!is_array($data)) {
            ...
            if ($unslashed !== $raw) { $data = json_decode($unslashed, true); }
        }
        if (!is_array($data)) {
            $repaired = self::repair_unescaped_html_css_strings($raw);
            if ($repaired !== $raw) { $data = json_decode($repaired, true); }
        }
        if (!is_array($data)) {
            return new WP_Error('invalid_json', 'JSON page không hợp lệ: ' . json_last_error_msg() . '. Gợi ý: HTML/CSS trong JSON phải escape dấu nháy kép.', ['status' => 400]);
        }
        if (!empty($extra['output_mode'])) {
            $data['output_mode'] = self::output_mode($extra['output_mode']);
            if (isset($data['page']) && is_array($data['page'])) { $data['page']['output_mode'] = $data['output_mode']; }
        }
    ...
    public static function validate(array $data)
    {
        ...
        foreach ($sections as $index => $section) {
            ...
            $html = $htmlN['value'];
            $css = $cssN['value'];
            $shortcode = $shortcodeN['value'];
            if ($htmlN['changed'] || $cssN['changed'] || $shortcodeN['changed']) { $out['warnings'][] = "Section {$id}: Đã tự sửa chuỗi bị double-escaped."; }

            if ($output_mode !== 'flatsome-native' && trim($html) === '') {
                $out['warnings'][] = "Section {$id} thiếu HTML và đã bị bỏ qua.";
                continue;
            }
            if ($output_mode !== 'flatsome-native' && stripos($html, 'pmedia-ai-block') === false) {
                $html = '<div class="pmedia-ai-block pm-page-block pm-page-block-' . esc_attr($id) . '">' . "\n" . trim($html) . "\n" . '</div>';
            }
            $normalized = self::normalize_section_css($css);
            $css = $normalized['css'];
            foreach ($normalized['warnings'] as $warning) { $out['warnings'][] = "Section {$id}: " . $warning; }

            $out['page']['sections'][] = [
                'id'=>$id,'type'=>$type,'title'=>sanitize_text_field($section['title'] ?? ''),'goal'=>sanitize_textarea_field($section['goal'] ?? ''),'html'=>$html,'css'=>$css,'shortcode'=>$shortcode,
    ...
    public static function to_shortcode(array $page): string
    {
        ...
        foreach ($page['sections'] as $section) {
            if ($output_mode === 'flatsome-native' && !empty($section['shortcode'])) {
                $content .= trim((string)$section['shortcode']) . "\n\n";
                continue;
            }
            if ($output_mode === 'flatsome-native') {
                $content .= self::fallback_native_shortcode($section) . "\n\n";
                continue;
            }
    ...
    private static function fallback_native_shortcode(array $section): string
    {
        ...
        $goal = esc_html($section['goal'] ?? '');
        $text = wp_strip_all_tags((string)($section['html'] ?? ''));
        $body = $text ?: $goal;
        return '[section class="pm-native-section pm-section-' . esc_attr($section['id'] ?? 'section') . '"]' . "\n"
            . '  [row]' . "\n"
            . '    [col span__sm="12"]' . "\n"
            . '      [ux_text]' . "\n"
            . '        <h2>' . $title . '</h2>' . "\n"
