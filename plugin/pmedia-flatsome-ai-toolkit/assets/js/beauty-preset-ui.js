(function () {
  ...
  document.addEventListener('click', async function (e) {
    ...
    e.preventDefault();
    var select = qs('pmfai-settings-beauty-preset') || document.querySelector('select[name="pmedia_flatsome_ai_toolkit_options[beauty_preset]"]');
    ...
    try {
      ...
      if (result.code) {
        alert(result.message || 'Apply preset thất bại.');
        return;
      }
      syncSettingsInputs(result.tokens || {});
      var box = qs('pmfai-beauty-preset-result');
      if (box) box.innerHTML = '<div class="pmfai-status-box is-success"><strong>Đã apply Beauty Preset: ' + esc(result.preset) + '</strong><br>Design System đã được cập nhật. Refresh trang hoặc bấm Lưu Settings nếu muốn kiểm tra lại form.</div>' + renderTokens(result.tokens || {});
    } finally {
      t.disabled = false;
      t.textContent = old;
    }
